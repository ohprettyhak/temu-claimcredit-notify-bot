...
const validateDateFormat = (date: string): void => {
  if (!DATE_FORMAT_REGEX.test(date)) {
    throw new Error(VALIDATION_ERROR_MESSAGES.INVALID_DATE_FORMAT(date));
  }
};

const validateTimeFormat = (time: string): void => {
  if (!TIME_FORMAT_REGEX.test(time)) {
    throw new Error(VALIDATION_ERROR_MESSAGES.INVALID_TIME_FORMAT(time));
  }
};

const validateTimezone = (timezone: string): void => {
  if (!DateTime.local().setZone(timezone).isValid) {
    throw new Error(VALIDATION_ERROR_MESSAGES.INVALID_TIMEZONE(timezone));
  }
};
...
export const toUTC = (date: string, time: string, timezone: string): string => {
  ...
  if (!dateTime.isValid) {
    throw new Error(VALIDATION_ERROR_MESSAGES.INVALID_DATETIME(date, time, timezone));
  }
